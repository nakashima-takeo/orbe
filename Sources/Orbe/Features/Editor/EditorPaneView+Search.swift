...
/// ファイル内検索。バーはスクロールバック検索と同じ部品（`SearchBar`）で、本文の右上（俯瞰の左）に浮く。
/// 検索の状態は `EditorSearch`、規則は Core。`SurfaceView+Search` と同形。
extension EditorPaneView {
  /// 検索バーを出す（既にあれば再フォーカス）。1 行以内の非空の選択があれば needle に入れて即検索する——
  /// 種は検索へ直接渡し、バーには表示として写す（SwiftUI の `onChange` は初回描画前の値では走らない）。
  /// 文書が無ければ何も起きない。
  func showSearch() {
    ...
    NSLayoutConstraint.activate([
      bar.trailingAnchor.constraint(
        equalTo: trailingAnchor, constant: -(overviewWidth + Theme.Space.beat)),
    ...
    if selection.length > 0 {
      ...
      if !seed.contains(where: \.isNewline) {
        bar.needle = seed
