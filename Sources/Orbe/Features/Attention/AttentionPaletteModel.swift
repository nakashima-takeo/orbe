...
///
/// 全ライブタブの agentState（waiting/done/working）を stateChangedAt 降順にフラット一覧し、
/// ↑↓ で選択・Enter/行タップでそのタブへ移動（WS activate＋タブ選択＋surface focus）・
...
@Observable final class AttentionPaletteModel {
  ...
  ///
  /// 並びは stateChangedAt 降順なので、開いている間に別タブが waiting / done へ変われば行が
  /// 先頭に挿し込まれ、以降の index が 1 つずつずれる。index を据え置くと ↵ が**選んだ覚えの
  /// ない別タブ**へ飛ぶので、選択は tabId を錨に追い直す（→ `ModalSelection.restore`。
  /// 裏の更新はユーザの意図ではないのでモダリティは奪わない）。錨が消えたときだけ範囲へ丸める。
  func setRows(_ rows: [AttentionRow]) {
    let anchor =
      self.rows.indices.contains(render.selected) ? self.rows[render.selected].tabId : nil
    self.rows = rows
    rebuild()
    if let anchor, let i = rows.firstIndex(where: { $0.tabId == anchor }) {
  ...
  func activate() {
    ...
    onFocusTab?(rows[render.selected].tabId)
  }
