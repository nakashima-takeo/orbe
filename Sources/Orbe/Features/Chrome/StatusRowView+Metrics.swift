...
/// タブ行の寸法計算（幅・自然幅）。描画（`StatusRowView`）から分離する。入力は body が読んだ
/// `TabStrip` 1 つで、model の配列を index で引かない。
...
extension StatusRowView {
  ...
  /// `strip.count` と同じ要素数。
  func tabWidths(_ strip: TabStrip, available: CGFloat) -> [CGFloat] {
    var widths = StatusTabLayout.widths(
      naturals: strip.cells.map(naturalWidth(of:)), segments: strip.ranges, available: available)
    if let e = model.editingIndex, widths.indices.contains(e) {
      let glyph = strip.cells[e].glyph
      widths[e] = min(
        Chrome.tabMaxWidth, max(editingNaturalWidth(hasGlyph: glyph != nil), Chrome.tabEditFloor))
    }
    return widths
  }
  ...
  func naturalWidth(of cell: TabStrip.Cell) -> CGFloat {
    naturalWidth(text: displayTitle(cell), hasGlyph: cell.glyph != nil)
  }
  ...
  func editingNaturalWidth(hasGlyph: Bool) -> CGFloat {
