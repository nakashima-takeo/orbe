...
/// 外半径:内半径 = 15:11.777、viewBox 32 座標を size へ等倍）。塗りは単色（メニューバーの
/// template image 相当＝前景モノクロ描画）。
...
struct OrbeMarkShape: Shape {
  ...
  #Preview("OrbeMarkGlyph") {
    HStack(spacing: Theme.Space.span) {
      OrbeMarkGlyph(size: 15, color: Color.theme.textPrimary)
      OrbeMarkGlyph(size: 24, color: Color.theme.textPrimary)
      OrbeMarkGlyph(size: 48, color: Color.theme.textPrimary)
    }
