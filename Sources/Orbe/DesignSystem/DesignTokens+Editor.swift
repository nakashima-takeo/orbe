...
/// エディター面（⌘E）のトークン。面のキー色は「どの面に居るか」を示す識別色で、背の印・分割中の
/// 焦点帯・位置ドットが共有する。`Theme` 本体の行数上限を避けて extension に置く。
extension Theme.Color {
  /// エディター面のキー色（amber）。
  static let faceEditor = editorDyn(light: 0xbf6a2e, dark: 0xe0a97c)
  /// 端末面のキー色。dark は textSecondary と偶然同値だが役割が違うので別トークン。
  ...
  /// エディター面のアイコン・kbd の文字。dark は kbKeyText と偶然同値だが役割が違うので別トークン。
...
extension Theme.Motion {
  ...
  static let faceSlideEasing: (p1: CGPoint, p2: CGPoint) = (
    CGPoint(x: 0.32, y: 0.72), CGPoint(x: 0, y: 1)
  )
  /// 背の地の切替（印 ⇄ グリップ）。
