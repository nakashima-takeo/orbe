...
enum L10n {
  /// 表の実体はファイルスコープの `l10nTable`（下）。UI 面が増えるほど育つ長大リテラルで、
  /// 型の複雑さ（type_body_length）とは別物のため型 body の外に置く。
  static let table = l10nTable
}

/// UI 文言の表本体（`L10n.table` からのみ参照）。
private let l10nTable: [L10nKey: (ja: String, en: String)] = [
  // MARK: Menu
  .menuHide: ("%@を隠す", "Hide %@"),
