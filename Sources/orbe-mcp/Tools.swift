  ...
  obj([
    ...
    (
      ...
      "閉じたセッションを休眠チケットとして該当 workspace（rootPath 一致。無ければログの名前で作る）へ"
        + "戻す（位置は新規タブと同じ規則＝同じ worktree の連の右端、無ければ末尾）。resume_agent と違い"
        + "起動しない——選択も前面化もせず、タブが mount されたとき resume で起きる（アクティブ workspace に"
        + "戻した分は次にいずれかのタブを選んだとき他の未 mount タブと順次、背景 workspace の分はその "
        + "workspace のアクティブ化で）。復元先が背景 workspace のとき、話しかけるには先に focus_tab / "
        + "activate_workspace で起こす（起こす前の prompt_agent は tab not mounted）。結果は id ごとに "
        + "restored / already-present / unknown（部分成功も成功・冪等）。多数を一度に戻すときはこのツール"
