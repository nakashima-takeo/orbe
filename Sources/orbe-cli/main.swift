...
// Orbe 自身を構成・操作する CLI（config / ws / tab / agent / session / wait）。control.sock（改行区切り JSON-RPC 2.0）へ 1 リクエスト
...
case "agent":
  runAgent(Array(args.dropFirst()))
case "session":
  runSession(Array(args.dropFirst()))
case "wait":
