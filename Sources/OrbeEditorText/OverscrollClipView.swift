...
/// 本文の外の点を末尾の位置として解くので、キャレットは文書の末尾へ行き、そのままドラッグで選択が伸びる。空き地の本文の
/// 列には I ビームを張る（上流は文書の高さまでしか張らない）。
final class OverscrollClipView: NSClipView {
  /// 最終行の上端（documentView の座標）。面が行の矩形から答える。
  var lastLineTop: () -> CGFloat? = { nil }
  /// 最終行より下の空き地のうち本文の列（ガターの右）。無ければ空。
  ...
  func updateBlankArea(gutterWidth: CGFloat) {
    let top = documentView?.frame.maxY ?? bounds.maxY
    let area =
      bounds.maxY > top
      ? NSRect(
        x: bounds.minX + gutterWidth, y: top, width: max(0, bounds.width - gutterWidth),
