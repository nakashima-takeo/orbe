...
///
/// wire 形は平坦な JSON で `origin` / `reason` は closed のときだけ現れる。型では直和 `Kind` で持ち、
/// opened に origin が付いた値を作れないようにする（そういう行は復号エラー＝読み飛ばし対象）。
public struct SessionEvent: Codable, Equatable {
  ...
  public enum Kind: Equatable {
    case opened
    case closed(origin: CloseOrigin, reason: String?)
  }
  ...
  public var closeOrigin: CloseOrigin? {
    if case .closed(let origin, _) = kind { return origin }
    return nil
  }

  /// closed の `reason`（無ければ nil）。
  public var closeReason: String? {
    if case .closed(_, let reason) = kind { return reason }
    return nil
  }
  ...
  private enum CodingKeys: String, CodingKey {
    case ts, event, workspace, cwd, agent, origin, reason
  }
  ...
  public init(from decoder: Decoder) throws {
    ...
    case "opened":
      guard !c.contains(.origin), !c.contains(.reason) else {
        throw DecodingError.dataCorruptedError(
          forKey: .event, in: c, debugDescription: "opened carries origin/reason")
      }
      kind = .opened
    case "closed":
      kind = .closed(
  ...
  public func encode(to encoder: Encoder) throws {
    ...
    case .closed(let origin, let reason):
