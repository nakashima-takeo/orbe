...
///
/// 壊れると何が起きるか: `orb tab send --json` の `seq` を `orb wait --after` に渡す手順が黙って
...
/// （`tab_closed`）がいつ割り込むかは決められない。seq はリテラルで書かず直前の応答から読み、
/// 隣接（`+ 1`）ではなく**前後関係**（自分の tab に絞った一致・大小）で言う。`+ 1` の厳密さは L1 が持つ。
extension ControlWireTests {
  ...
  private func done(_ tabId: Int, message: String? = nil, sessionId: String? = nil) -> ControlEvent
  {
    .agentState(tabId: tabId, state: "done", message: message, sessionId: sessionId)
  }

  // MARK: - seq

  /// kind・tab を問わず 1 本の seq が進み、event 応答の `event.seq` と最上位 `seq` は
  /// そのイベント自身の seq。
  func testEveryEventCarriesTheNextSeqOnOneCounter() {
    ...
    let events: [ControlEvent] = [
      done(8201), .title(tabId: 8202, title: "t"), .pwd(tabId: 8203, path: "/p"),
      .tabClosed(tabId: 8204),
    ]

    for (index, next) in events.enumerated() {
      armWait(wire, id: 10 + index, params: ["tabId": next.tabId])
  ...
  func testSuccessResponsesCarryTheHistoryPositionAndErrorsDoNot() {
    ...
    let successes: [(String, [String: Any])] = [
      ("list_tabs", [:]),
      ("send_text", ["tabId": tab, "text": "hi"]),
      ("send_key", ["tabId": tab, "key": "enter"]),
      ("spawn", [:]),
    ...
    let failure = wire.request(id: 21, method: "send_text", params: ["tabId": tab])
  ...
  func testAgentStateEventCarriesMessageAndSessionIdWhenReported() {
    ...
    armWait(wire, id: 1, params: ["tabId": 8206])
    ...
    armWait(wire, id: 2, params: ["tabId": 8206])
    ControlServer.shared.emit(done(8206))
    let bare = event(wire.nextResponse())
    XCTAssertEqual(bare?["value"] as? String, "done", "前提: 同じ tab の agent_state")
  ...
  func testAfterReplaysAnEventThatAlreadyHappenedWithoutWaiting() {
    ...
    ControlServer.shared.emit(.pwd(tabId: 8208, path: "/later"))
    let latest = currentSeq(wire, id: 2)

    let response = wire.request(
      id: 3, method: "wait_for_event", params: ["after": before, "tabId": 8207])

    XCTAssertEqual(event(response)?["tabId"] as? Int, 8207, "既に起きたイベントで即返る")
  ...
  func testAfterReturnsTheEarliestMatchNotTheLatest() {
    ...
    ControlServer.shared.emit(
      .agentState(tabId: 8209, state: "working", message: nil, sessionId: nil))
    ...
    let response = wire.request(
      ...
      params: ["after": before, "tabId": 8209, "value": "done"])
  ...
  func testAfterWithoutAPastMatchWaitsForTheNextEvent() {
    let wire = startWire(target: FakeControlTarget())
    ControlServer.shared.emit(
      .agentState(tabId: 8210, state: "working", message: nil, sessionId: nil))
    let before = currentSeq(wire, id: 1)

    armWait(wire, id: 2, params: ["after": before, "tabId": 8210, "value": "done"])
  ...
  func testAfterBeyondTheLatestSeqIsRejectedWithoutArming() {
    ...
    ControlServer.shared.emit(.pwd(tabId: 8211, path: "/x"))
  ...
  func testAfterOlderThanRetainedHistoryIsEvicted() {
    ...
    let capacity = 4096
    for _ in 0...capacity { ControlServer.shared.emit(.pwd(tabId: 8212, path: "/flood")) }
    let latest = currentSeq(wire, id: 2)
    ...
    // 保持範囲の内側は replay できる。最古ぴったりは他テストの残骸（tab_closed）が割り込むと
    ...
    ControlServer.shared.emit(.pwd(tabId: 8212, path: "/after"))
  ...
  func testValueFilterWakesOnlyOnTheMatchingValue() {
    let wire = startWire(target: FakeControlTarget())
    armWait(wire, id: 1, params: ["tabId": 8214, "value": "done"])

    ControlServer.shared.emit(
      .agentState(tabId: 8214, state: "working", message: nil, sessionId: nil))
    ControlServer.shared.emit(.title(tabId: 8214, title: "done!"))
  ...
  func testValueClearWakesOnTheReportDisappearance() {
    let wire = startWire(target: FakeControlTarget())
    armWait(wire, id: 1, params: ["tabId": 8215, "value": "clear"])

    ControlServer.shared.emit(done(8215))
    wire.barrier()  // 状態語のある遷移では起きない

    ControlServer.shared.emit(.agentState(tabId: 8215, state: nil, message: nil, sessionId: nil))
    let response = wire.nextResponse()
