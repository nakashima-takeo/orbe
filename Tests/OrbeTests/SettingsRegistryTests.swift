...
final class SettingsRegistryTests: XCTestCase {
  ...
  /// devFeatures はビルド種別 = swift test は dev 扱いで on）。
  func testDefaultValues() {
    ...
    XCTAssertEqual(
      SettingsRegistry.descriptor(.devFeaturesEnabled).defaultValue(), .bool(true),
      "チャネルは release がオプトイン。素の swift test は -DORBE_RELEASE を焼かない＝dev で既定 on")
  }
