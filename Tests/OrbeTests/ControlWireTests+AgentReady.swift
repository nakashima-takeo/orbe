...
/// 落ちれば resume の鍵を `list_tabs` から拾い直す手順が戻る。時間切れが launch を捨てれば、
/// 実際には開いているタブの tabId を呼び出し側が知る手段が無くなる。
extension ControlWireTests {
  ...
  private func idle(_ tabId: Int, sessionId: String? = nil) -> ControlEvent {
    .agentState(tabId: tabId, state: "idle", message: nil, sessionId: sessionId)
  }
  ...
  func testIdleReportingAgentWaitsForItsFirstIdleAndReturnsTheSession() {
    ...
    // Fake は spawn を tab 4344、resume を tab 4347 に起こす。
    for (index, entry) in [("spawn_agent", 4344), ("resume_agent", 4347)].enumerated() {
      let id = 1 + index * 10
      ...
      ControlServer.shared.emit(
        .agentState(tabId: entry.1, state: "working", message: nil, sessionId: "s"))
      ControlServer.shared.emit(idle(entry.1 + 1, sessionId: "other"))
      ControlServer.shared.emit(.tabClosed(tabId: entry.1 + 1))
      wire.barrier()  // idle 以外の状態・別タブの idle / 消滅では起きない
      ...
      XCTAssertEqual(launched?["tabId"] as? Int, entry.1, "\(entry.0) の launch が載る")
  ...
  func testIdleWithoutSessionIdOmitsAgentSessionId() {
    ...
    ControlServer.shared.emit(idle(4344))
  ...
  func testReadyTimeoutKeepsTheLaunchAndFlagsTimedOut() {
    ...
    XCTAssertEqual(launched?["tabId"] as? Int, 4344, "時間切れでも開いたタブの宛先を返す")
  ...
  /// 待機中にそのタブが閉じたら -32000 "agent exited"（起動失敗で即終了した形）。
  func testTabClosedWhileWaitingForReadyIsAgentExited() {
    let fake = FakeControlTarget()
    let wire = startWire(target: fake)

    launch(wire, id: 1, method: "spawn_agent", command: "claude")
    ControlServer.shared.emit(.tabClosed(tabId: 4344))
    let response = wire.nextResponse()
