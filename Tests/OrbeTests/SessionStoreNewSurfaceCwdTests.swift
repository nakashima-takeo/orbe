...
/// SessionStore.newSurfaceCwd(inWorkspaceAt:) の純ドメイン契約を固定する。
///
/// newSurfaceCwd(inWorkspaceAt:) は新規タブ/エージェント起動の初期 cwd を必ず確定させる：アクティブペインの cwd
...
final class SessionStoreNewSurfaceCwdTests: OrbeTestCase {
  ...
  func testZeroTabsFallsBackToWorkspaceRootPath() {
    ...
    XCTAssertEqual(store.newSurfaceCwd(inWorkspaceAt: 0), "/tmp/ws-root")
  }
  ...
  func testActivePaneCwdWinsOverRootPath() {
    ...
    XCTAssertEqual(store.newSurfaceCwd(inWorkspaceAt: 0), "/tmp/pane-cwd")
  }
