...
extension WindowControllerWorkspaceTests {
  ...
  func testPaletteShowsDormantRestoredAgentsAsDistinctKind() throws {
    ...
    func rollup(of name: String) -> [(state: String, count: Int)]? {
      items.first { $0.name == name }.map(\.live.rollup).flatMap { $0.isEmpty ? nil : $0 }
    }
    ...
    XCTAssertEqual(items.first { $0.name == "sleepers" }?.live.dormant, true)
    XCTAssertEqual(items.first { $0.name == "quiet" }?.live.dormant, true)
    XCTAssertEqual(items.first { $0.name == "main" }?.live.dormant, false)
  }
  ...
  func testPaletteShowsLiveAndDormantChipsTogetherInCanonicalOrder() throws {
    ...
    XCTAssertFalse(item.live.dormant, "live タブが 1 枚でもあれば行は減光しない")
    XCTAssertEqual(
      item.live.rollup.map(\.state), ["working", "waiting", "done", "idle", "dormant"])
    XCTAssertEqual(item.live.rollup.map(\.count), [1, 1, 1, 1, 2])
