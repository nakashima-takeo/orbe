...
extension SettingsPaletteTests {
  ...
  /// root は絞り込み入力欄あり（filter）。クエリ空なら全 17 行（スコープ＋15 設定＋言語）を出す。
  func testRootHasFilterFieldAndShowsAllRowsWhenEmpty() {
    ...
    XCTAssertEqual(p.render.rows.count, 17, "クエリ空なら全行（スコープ＋15 設定＋言語）")
  }
  ...
  func testClearingQueryRestoresAllRows() {
    ...
    p.render.query = ""
    p.render.onQueryChange()
    XCTAssertEqual(p.render.rows.count, 17, "クエリを消すと全行へ戻る")
  }
  ...
  func testFilteredDrillInAndReturnRestoresSameSettingRow() {
    ...
    XCTAssertEqual(p.render.rows.count, 17, "全行へ戻る")
