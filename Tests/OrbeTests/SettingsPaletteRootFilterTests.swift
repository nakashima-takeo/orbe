...
extension SettingsPaletteTests {
  ...
  /// root は絞り込み入力欄あり（filter）。クエリ空なら全 16 行（スコープ＋14 設定＋言語）を出す。
  func testRootHasFilterFieldAndShowsAllRowsWhenEmpty() {
    ...
    XCTAssertEqual(p.render.rows.count, 16, "クエリ空なら全行（スコープ＋14 設定＋言語）")
  }
  ...
  func testClearingQueryRestoresAllRows() {
    ...
    p.render.query = ""
    p.render.onQueryChange()
    XCTAssertEqual(p.render.rows.count, 16, "クエリを消すと全行へ戻る")
  }
  ...
  func testFilteredDrillInAndReturnRestoresSameSettingRow() {
    ...
    XCTAssertEqual(p.render.rows.count, 16, "全行へ戻る")
