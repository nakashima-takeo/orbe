...
final class WindowControllerOpacityTests: OrbeTestCase {
  ...
  func testActiveEffectiveTracksActiveWorkspaceOverride() {
    ...
    wc.createWorkspace(name: "other", rootPath: wc.store.activeTabCwdOrHome())  // 上書き無しの新 WS がアクティブ
    XCTAssertEqual(
  ...
  func testCreateWorkspaceRegeneratesConfigToGlobal() {
    ...
    // 上書き無しの新 WS がアクティブに
    wc.createWorkspace(name: "other", rootPath: wc.store.activeTabCwdOrHome())
    XCTAssertEqual(
