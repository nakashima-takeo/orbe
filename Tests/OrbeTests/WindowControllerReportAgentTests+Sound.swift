...
/// `report_agent` から通知音までの配達経路（ファイル分割の拡張）。
/// 鳴らす**判断**そのものは `AgentSoundDecisionTests` が純関数で総当たりするので、ここで測るのは
/// 「発火点が正しいか（waiting / done への実変化だけか）」「見ているタブで抑制されるか」
/// 「どの workspace の設定を読むか」の 3 点。再生層は隔離ハーネスがフェイクへ差してある。
extension WindowControllerReportAgentTests {
  ...
  func testSoundReadsOriginWorkspaceOverride() throws {
    ...
    override[SettingKeys.notificationSound] = NotificationSound.hagane
    override[SettingKeys.notificationSoundVolume] = 30
    wc.current.settingsOverride = override

    wc.controlReportAgent(pane: pane, agent: "claude", state: "done", sessionId: nil, message: nil)
    XCTAssertEqual(
      sound.played, [SoundPlayerFake.Played(family: .hagane, event: .done, volume: 30)],
      "その workspace の上書きが global に勝つ")
  }
}
