...
extension SettingsPaletteTests {
  ...
  func testMenuBarNotificationDurationRowAppearsWithDefault() {
    let p = model()
    XCTAssertTrue(p.render.rows[durationRow].label.contains("メニューバー通知の表示時間"))
    XCTAssertTrue(p.render.rows[durationRow].label.contains("40秒"), "未設定なら既定 40 秒を単位つきで表示")
    XCTAssertFalse(p.render.rows[durationRow].chevron, "stepper 行は chevron を持たない")
  }
  ...
  func testMenuBarNotificationDurationRowSeparatesTheUnitInEnglish() {
    ...
    XCTAssertTrue(p.render.rows[durationRow].label.contains("Menu Bar Notification Duration"))
    XCTAssertTrue(p.render.rows[durationRow].label.contains("40 s"), "英語は数と単位の間を空ける")
  }

  func testMenuBarNotificationDurationIncrement() {
    let p = model(menuBarNotificationDuration: 40)
    let applied = captureApply(p)
    p.render.selected = durationRow
    _ = p.render.onRight()  // → で 5 秒増
    XCTAssertEqual(applied()?[SettingKeys.menuBarNotificationDuration], 45)
    XCTAssertTrue(p.render.rows[durationRow].label.contains("45秒"))
  }
  ...
  func testMenuBarNotificationDurationDecrement() {
    ...
    XCTAssertTrue(p.render.rows[durationRow].label.contains("35秒"))
  }
  ...
  func testMenuBarNotificationDurationClampHigh() {
    ...
    XCTAssertTrue(p.render.rows[durationRow].label.contains("180秒"))
  }
  ...
  func testMenuBarNotificationDurationClampLow() {
    ...
    XCTAssertTrue(p.render.rows[durationRow].label.contains("5秒"))
  }
  ...
  func testMenuBarNotificationDurationWorkspaceOverrideAndInherit() {
    ...
    XCTAssertTrue(p.render.rows[durationRow].label.contains("90秒"), "上書きが実効値として出る")
    XCTAssertFalse(p.render.rows[durationRow].inherited, "上書き中は継承マーク無し")

    p.render.selected = durationRow
    p.render.onDelete()  // 上書きを解除して global 継承へ戻す
    XCTAssertTrue(p.render.rows[durationRow].label.contains("40秒"), "global の 40 秒を継承")
    XCTAssertTrue(p.render.rows[durationRow].inherited)
    XCTAssertEqual(p.render.rows[durationRow].detail, "（継承）")
  }
}
