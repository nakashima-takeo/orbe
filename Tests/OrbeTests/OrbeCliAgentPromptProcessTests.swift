...
/// × **状態を報告し返す偽 claude** で固定する。偽 claude はペインに注入された `ORBE_REPORT_BIN`
...
final class OrbeCliAgentPromptProcessTests: OrbeTestCase {
  ...
  /// 偽 claude を仕込んだサーバを立て、ready まで待った spawn の応答とペイン id を返す。
  private func spawnReady(file: StaticString = #filePath, line: UInt = #line) throws -> ReadySpawn {
    ...
    let pane = try XCTUnwrap(
      spawned["paneId"] as? Int, "spawn が paneId を返さない", file: file, line: line)
    XCTAssertEqual(
      spawned["ready"] as? Bool, true, "前提: 偽 claude の idle で ready", file: file, line: line)
    return ReadySpawn(control: control, pane: pane, spawned: spawned)
  }
  ...
  func testSpawnWaitsForTheFirstIdleAndReportsTheSession() throws {
    ...
    XCTAssertTrue(
      plain.stdout.hasPrefix("spawned claude in pane ")
  ...
  /// idle が来なければ `--timeout-ms` で exit 124。タブは開いているので宛先（paneId）は捨てず、
  ...
  func testSpawnReadyTimeoutExits124ButKeepsTheLaunch() throws {
    ...
    XCTAssertNotNil(payload["paneId"] as? Int, "時間切れでも開いたタブの paneId を返す")
    ...
    XCTAssertTrue(
      plain.stdout.hasPrefix("spawned claude in pane ") && !plain.stdout.contains(" ready"),
  ...
  /// `--stdin` は `pane send` と同じ規則で本文を取る。`<pane>` と `--text` / `--stdin` のちょうど一方は
  /// 必須で、欠けは socket に触れる前の usage エラー。
  func testPromptTakesStdinAndRequiresAPaneAndOneTextSource() throws {
    for args in [
  ...
  func testPromptTimeoutExits124() throws {
    ...
    let plain = control.orb([
      "agent", "prompt", "\(pane)", "--text", "slow:a", "--timeout-ms", "500",
    ])
