...
/// ここが測るのは状態機械の帰結——slot・`list_panes`・`snapshot()`・チケット消費の 4 面。
///
/// 壊れると、①偽の報告が未消費の復元チケットを書き換えて resume の鍵（sessionId）が失われる
/// （再起動後に二度と resume できず、しかも `list_panes` は偽値を返すので呼び手は気づけない）、
/// ②CLI をまたいだ sessionId の生き残りで「codex ＋ claude 由来 id」という resume 不能な組が
/// 永続記録に載る、③消費済みのペインが休眠へ巻き戻り休眠数が二重計上される、のいずれかに倒れる。
///
...
extension WindowControllerReportAgentTests {
  ...
  /// アクティブ workspace（素のシェル）＋ **一度も activate していない**背景 workspace（agent 葉 1 枚）で
  /// 起動する。背景側は mount されないのでペインは `.dormant`（未消費チケット）のまま留まる。
  /// 分割した拡張ファイル（+StateEmit）からも使うため internal。
    ...
    -> DormantTicketFixture
  {
    ...
    let file = WorkspacesFile(
      ...
      workspaces: [
        ...
        WorkspaceState(
          ...
          tabs: [TabState(tree: .leaf(cwd: "/w", agent: ticket), explicitTitle: nil)]),
      ])
    ...
    XCTAssertEqual(pane.agentSlot, .dormant(ticket), "前提: 復元直後は未消費のチケット")
    return DormantTicketFixture(wc: wc, tab: tab, pane: pane, ticket: ticket)
  }

  /// `list_panes` から当該ペインの行を引く。
  private func listPaneRow(_ wc: WindowController, paneId: Int) -> [String: Any]? {
    wc.controlListPanes().first { $0["paneId"] as? Int == paneId }
  }

  /// snapshot の葉が持つ agent セッション。cwd は実 surface だと `currentPwd` が入って
  /// 実行環境に依存するため、比べるのは agent だけにする。
  private func snapshotAgent(_ tab: TerminalController) throws -> AgentSession? {
    guard case .leaf(_, let agent) = tab.snapshot() else {
      XCTFail("単一ペインのタブなので snapshot は葉になる: \(tab.snapshot())")
      return nil
    }
    return agent
  }
  ...
  /// 休眠チケット宛の報告は完全に破棄される——slot は Equatable で不変のまま、`list_panes` にも
  /// 偽の CLI 名・sessionId・状態が一切漏れない。dormant ペインは surface 未生成で報告主の
  /// プロセスが存在しえないため、届く報告は必ず偽物。
  func testDormantTicketDiscardsForgedReportOnSlotAndListPanes() throws {
    ...
    XCTAssertEqual(f.pane.agentSlot, .dormant(f.ticket), "報告は破棄され slot は完全に不変")
    let row = try XCTUnwrap(listPaneRow(f.wc, paneId: f.pane.id))
    XCTAssertTrue(row["agentState"] is NSNull, "休眠ペインに報告状態は載らない")
  ...
  func testForgedReportDoesNotStopDormantTicketFromResuming() throws {
    let f = try makeControllerAndDormantTicket(command: "claude", sessionId: "resume-1")
    f.wc.controlReportAgent(
      pane: f.pane, agent: "codex", state: "working", sessionId: "forged", message: nil)

    f.tab.recordMaterializationStarted()

    XCTAssertEqual(
      f.pane.agentSlot, .live(session: f.ticket, report: nil), "チケットの同一性がそのまま live へ移る")
    XCTAssertEqual(f.pane.initialCommand, "claude --resume resume-1", "解決した resume 指示が spawn に効く")
    XCTAssertNotNil(
      f.pane.initialEnv["PATH"], "起動環境には PATH が入る（値は login shell の probe 次第なので見ない）")
  }

  // MARK: - .dormant × clear（破棄）

  /// 休眠チケット宛の clear も破棄される。clear が届いても永続記録（snapshot の agent）は
  ...
  func testDormantTicketDiscardsClearAndKeepsTicketOnEveryObservedFace() throws {
    ...
    XCTAssertEqual(f.pane.agentSlot, .dormant(f.ticket), "clear も破棄され slot は完全に不変")
    let row = try XCTUnwrap(listPaneRow(f.wc, paneId: f.pane.id))
    XCTAssertTrue(row["agentState"] is NSNull)
    XCTAssertEqual(row["agentSessionId"] as? String, "resume-1")
    XCTAssertEqual(try snapshotAgent(f.tab), f.ticket, "休眠のままの保存でチケットの同一性が消えない")
  }
  ...
  func testClearDoesNotDivertUnresolvableTicketFromFallingBackToShell() throws {
    ...
    f.wc.controlReportAgent(
      pane: f.pane, agent: "unknown", state: "clear", sessionId: nil, message: nil)
    XCTAssertEqual(f.pane.agentSlot, .dormant(f.ticket), "clear はチケットを消費も破壊もしない")

    f.tab.recordMaterializationStarted()

    XCTAssertEqual(f.pane.agentSlot, .none, "resume 非対応は消費で素のシェルへ落ちる")
    XCTAssertNil(f.pane.initialCommand, "起動指示は立たない")
  }

  // MARK: - .none × clear（no-op）

  /// 一度も報告のないペインへ clear が届いても何も起きない（SessionEnd の hook が未報告の
  /// ペインへ回り込む経路。壊れないことだけが契約）。
  func testClearOnAgentlessPaneIsNoOp() throws {
    let (wc, pane) = try makeControllerAndPane()

    wc.controlReportAgent(pane: pane, agent: "claude", state: "clear", sessionId: nil, message: nil)

    XCTAssertEqual(pane.agentSlot, .none)
    let row = try XCTUnwrap(listPaneRow(wc, paneId: pane.id))
    XCTAssertTrue(row["agentState"] is NSNull)
  ...
  /// `list_panes`（resume の鍵の読み口）と `snapshot()`（再起動をまたぐ永続記録）の両方へ届く。
  func testFirstReportEstablishesIdentityAndPersistsIt() throws {
    ...
    XCTAssertEqual(pane.agentSlot.session, session, "報告した CLI 名と sessionId が同一性になる")
    XCTAssertEqual(pane.agentState, "working", "報告が live の report として載る")
    XCTAssertEqual(
      listPaneRow(wc, paneId: pane.id)?["agentSessionId"] as? String, "s-1")
    XCTAssertEqual(try snapshotAgent(wc.current.tabs[0]), session, "報告由来の同一性も永続記録に載る")
  }
  ...
  func testFirstReportWithoutSessionIdGoesLiveButPersistsNothing() throws {
    ...
    XCTAssertEqual(
      pane.agentSlot.session, AgentSession(command: "claude", sessionId: nil), "CLI 名だけが立つ")
    XCTAssertEqual(pane.agentState, "working")
    XCTAssertTrue(listPaneRow(wc, paneId: pane.id)?["agentSessionId"] is NSNull)
    XCTAssertNil(try snapshotAgent(wc.current.tabs[0]), "sessionId 未確定の同一性は永続化しない")
  }
  ...
  func testSessionIdIsStickyWithinOneCliAndDroppedWhenTheCliChanges() throws {
    ...
    wc.controlReportAgent(
      pane: pane, agent: "claude", state: "working", sessionId: "a", message: nil)
    wc.controlReportAgent(
      pane: pane, agent: "claude", state: "waiting", sessionId: nil, message: nil)
    XCTAssertEqual(pane.agentSlot.session?.sessionId, "a", "同一 CLI なら運ばない報告でも鍵は生き残る")
    XCTAssertEqual(listPaneRow(wc, paneId: pane.id)?["agentSessionId"] as? String, "a")

    wc.controlReportAgent(
      pane: pane, agent: "claude", state: "waiting", sessionId: "b", message: nil)
    XCTAssertEqual(pane.agentSlot.session?.sessionId, "b", "新値が来れば上書きする")

    // state は直前と同値（同値枝）のまま CLI だけ入れ替える。
    wc.controlReportAgent(
      pane: pane, agent: "codex", state: "waiting", sessionId: nil, message: nil)
    XCTAssertEqual(
      pane.agentSlot.session, AgentSession(command: "codex", sessionId: nil),
      "CLI が変われば command を上書きしたうえで旧 id を捨てる")
    XCTAssertTrue(listPaneRow(wc, paneId: pane.id)?["agentSessionId"] is NSNull)
    XCTAssertNil(try snapshotAgent(wc.current.tabs[0]), "鍵を失った同一性は永続化しない")
  }
  ...
  func testFirstHookAfterTicketConsumptionKeepsTheResumeKey() throws {
    let f = try makeControllerAndDormantTicket(command: "claude", sessionId: "resume-1")
    f.tab.recordMaterializationStarted()
    XCTAssertEqual(f.pane.agentSlot, .live(session: f.ticket, report: nil), "前提: 消費済み・初回報告前")

    f.wc.controlReportAgent(
      pane: f.pane, agent: "claude", state: "working", sessionId: nil, message: nil)

    XCTAssertEqual(f.pane.agentSlot.session, f.ticket, "同一 CLI の初回報告はチケットの鍵を残す")
    XCTAssertEqual(
      listPaneRow(f.wc, paneId: f.pane.id)?["agentSessionId"] as? String, "resume-1")
    XCTAssertEqual(try snapshotAgent(f.tab), f.ticket, "永続記録も鍵を保つ")
  }
  ...
  func testConsumedTicketNeverReturnsToDormant() throws {
    ...
    f.wc.controlReportAgent(
      pane: f.pane, agent: "claude", state: "clear", sessionId: nil, message: nil)
    XCTAssertEqual(f.pane.agentSlot, .none, "clear は無へ戻す（休眠へは戻さない）")
    XCTAssertEqual(workspace.dormantAgentCount(), 0)

    f.wc.controlReportAgent(
      pane: f.pane, agent: "claude", state: "working", sessionId: "new", message: nil)
    XCTAssertEqual(
      f.pane.agentSlot.session, AgentSession(command: "claude", sessionId: "new"),
      "再報告は休眠を経ずに live 化する")
    XCTAssertEqual(workspace.dormantAgentCount(), 0)
  }
}
