...
import { posixBasename } from "../utils/path.js";
...
// query との一致品質。完全一致=2 > 前方一致=1 > 部分一致/その他=0。
// partialCmd は path 時 basename 化済み。folder 名に付く末尾スラッシュは剥がして比較する。
// 空 query（undefined / ""）は全候補を中立(0)に置き、priority で並べる。
const matchQuality = (name: string, partialCmd: string | undefined): number => {
  if (partialCmd == null || partialCmd.length === 0) return 0;
  const n = name.toLowerCase().replace(/\/$/, "");
  const p = partialCmd.toLowerCase();
  if (n === p) return 2;
  if (n.startsWith(p)) return 1;
  return 0;
};

// 補完候補の合成順: 一致品質(完全>前方>部分) 降順 → priority 降順 → 名前長 昇順 → 元順序で安定。
// 名前長キーは query 入力時のみ効かせる（一致した候補群の副次キー）。空 query の全列挙は
// 供給元（spec / 列挙）の順を安定保持し、priority 同値の並びを名前長で崩さない。
const compareSuggestions = (a: Suggestion, b: Suggestion, partialCmd: string | undefined): number => {
  const qa = matchQuality(a.name, partialCmd);
  const qb = matchQuality(b.name, partialCmd);
  if (qa !== qb) return qb - qa;
  if (a.priority !== b.priority) return b.priority - a.priority;
  if (partialCmd != null && partialCmd.length > 0) return a.name.length - b.name.length;
  return 0;
};
...
export const getSubcommandDrivenRecommendation = async (
  ...
  if (partialToken?.isPath) {
    partialCmd = partialToken.isPathComplete ? "" : posixBasename(partialCmd ?? "");
  }
          ...
          removeHomeDirectorySuggestion(
            removeAcceptedSuggestions(
              adjustPathSuggestions(
                suggestions.sort((a, b) => compareSuggestions(a, b, partialCmd)),
                partialToken,
                lastToken,
                shell,
              ),
