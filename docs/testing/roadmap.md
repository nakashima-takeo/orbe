...
| 1 | wire 契約 | 制御プロトコルの語を socketpair 上の実 `Connection` で固める（`ControlWireTests` 4 ファイル・40 本）。method 名・params キー・エラーコード・`wait_for_event`・framing・不正入力。前提として `ControlServer` に `adopt(fd:)` を切り出し、不正入力へ `-32700` / `-32600` を返すよう直した。エラーコードの語彙は `docs/spec/control-api.md` の「エラー」節と 1 対 1。#50 #62 | 0 | 完了 | — |
