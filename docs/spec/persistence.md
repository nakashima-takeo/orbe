...
互換: 旧スキーマは無損失で読み、次回保存で現行形式へ置き換わる（タブ構成・cwd・エージェントを失わない）。後から足したフィールドは欠落を許容し、`editor` は「あるが読めない」も既定へ落とす——タブ 1 枚の異常でファイル全体を落とさないため。設定層（global・workspace 上書きとも）は読めない 1 キーだけを落として残りを活かす——1 項目の異常で層ごと消さないため。値域を持つ項目（`theme` 等）は範囲外の値もそのまま層へ載せ、実効値の解決で既定へ落とす（値域は書き込み経路の `SettingsRegistry` 検証が守るので、範囲外の値はそもそも入らない）。旧 camelCase からの移行だけは global `settings.json` の `theme` を型付きで読むため、範囲外の値が既定値として層に載る——workspace 上書きとの差が出る値が実在しないので揃えていない。壊れている・非互換バージョン・空 JSON は既定の単一 workspace で fallback。
