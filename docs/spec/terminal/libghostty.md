...
- **`OPEN_URL` は URL の由来を `kind` で区別する**: 正規表現で検出したリンク（`UNKNOWN`／`TEXT`／`HTML`）と OSC 8 ハイパーリンク（`OSC8`）。OSC 8 は表示文字列と別の対象を指せる端末出力なので、host が信頼しない経路に通す（→ [links](links.md)）。libghostty は macOS で OSC 8 のフォールバックオープンを持たず、host が未処理を返しても開かれない。OSC 8 の検出自体は config `link-osc8` が持つ。
