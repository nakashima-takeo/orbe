...
- **クリップボード**: 端末アプリ発の読み取り（OSC 52 read・Kitty clipboard read）は Orbe 既定の `clipboard-read = deny`（→ [config](../platform/config.md)）で libghostty が拒否し、host には届かない——クリップボード内容の情報漏洩を防ぐため。このとき OSC 52 の読み取り要求には応答しないので、応答を待つ端末アプリ（SSH 先の nvim の OSC 52 provider 等）は待ち時間の後に空として扱う。host に届く読み取りはユーザー発のペーストとその型一覧で、扱う表現は `text/plain` だけ。`PRIMARY`（X11 の選択クリップボード）の読み取りは非対応と答える——macOS に無いものの読み取りに general の中身を渡して漏らさない（`SELECTION` は ⌘⇧V の `paste_from_selection` が使うので general に向ける）。Orbe は確認 UI を持たないので、確認が要る操作のうち通すのはペーストだけ（確認済みとして完了）で、それ以外は拒否する。ユーザーが `clipboard-read = ask` にすると端末アプリ発の読み取りがこの確認に回り、中身は渡らないが応答は `deny` と異なる——OSC 52 は中身があれば空の応答・空なら無応答、データだけを求める Kitty read は中身があれば EPERM・空なら成功（型一覧も併せて求める read は空でも EPERM）で、クリップボードが空かどうかが端末アプリに分かる（型一覧だけの Kitty read は確認を経ず応答される）。`ask` でも ⌘V に続く Kitty paste event（mode 5522）のパスワード付き read は許可済みとして確認なしで通る。既定を `deny` にしているのは、この差も見せないため。書き込みも同じ論理で、確認が要る書き込み（user が `clipboard-write = ask` にした場合）は通さない。書き込みは宛先（`PRIMARY` 等）によらず通常のクリップボードに置く——nvim は `*` レジスタを OSC 52 の `PRIMARY` で書くので、捨てると `"*y` が効かなくなる。書き込みでテキストと見なすのは macOS が plain text と解する MIME（`text/plain`・`text/plain;charset=utf-8` 等。Kitty write の MIME は正規化されず端末アプリの書いた名前のまま届くため、判定は macOS の型システムに委ねる）で、最初のテキスト表現を NSPasteboard へ置き、テキスト表現が無い書き込みは無視する（クリップボードを消さない）。表記の揺れをどこまで吸収するかも macOS の型解決に委ね、Orbe は MIME 文字列を自前で分解しない——`text/plain; charset=UTF-8` のように `;` の後に空白を含む MIME は `UTType(mimeType:)` が動的型に解決してテキストと判定されず、その書き込みは無視される（クリップボードは変わらない）。例外は UTF-16 系の plain text（`text/plain;charset=utf-16` 等）で、macOS はテキストと解するが Orbe は本文を UTF-8 としてしか読まないため、テキスト表現とは見なさない——その MIME だけの書き込みはクリップボードを変えない（NUL 混じりの文字列を置かない）。
  - **制約: Kitty clipboard protocol の paste event（mode 5522）を有効にした端末アプリには ⌘V でペーストできない。** mode 5522 では ⌘V で libghostty が、host から受け取った型一覧と使い捨てパスワードを端末アプリへ送り、端末アプリがそのパスワード付きの OSC 5522 read で中身を取りに来る。libghostty は `clipboard-read = deny` のとき、パスワードによる許可を見る前にこの read を拒否する。端末アプリには「クリップボードが空か」「どの型があるか」も見せない方針を優先した結果として受け入れている。ユーザーが `~/.config/ghostty` で `clipboard-read` を変えればこの限りではない。
- **ベル**（`RING_BELL`）は警告音のみで、視覚表現を持たない。
- **URL／ファイルパスのオープン**（`OPEN_URL`）は host が処理し macOS の作法で開く。正規表現で検出したリンクはそのまま開き、OSC 8 ハイパーリンクは端末出力を信頼しない 3 段判定（allow / confirm / block）を通す。規則は [links](links.md)。
- **マウス**: 左/右/中および拡張ボタンを libghostty へ転送する（マウスレポートを使う TUI のため）。フォーカス移動は左クリックのみ。**選択するだけ（ドラッグ選択・ダブル/トリプルクリック・⌘A・`right-click-action` による右クリックでの単語／リンク選択）で NSPasteboard に入り、中クリックで NSPasteboard の文字列をペーストする**（Orbe 既定の `copy-on-select = clipboard`・`middle-click-action = clipboard-paste`。→ [config](../platform/config.md)）——選択の 1 アクションでクリップボードに入る手順の短さを Orbe の価値として保つ。右クリックは選択外の単語／リンクを選択して同じ経路を通る。Orbe はコンテキストメニューを出さないので、メニューは出ないままその単語／リンクが選択表示になり、クリップボードもそれに置き換わる（受け入れている挙動）。マウス位置は tracking area で伝える（enter で viewport 内に位置確立・exit で範囲外座標）。
