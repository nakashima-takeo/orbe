...
- **マウス**: 左/右/中および拡張ボタンを libghostty へ転送する（マウスレポートを使う TUI のため）。フォーカス移動は左クリックのみ。**選択するだけ（ドラッグ選択・⌘A）で NSPasteboard に入り、中クリックで NSPasteboard の文字列をペーストする**（Orbe 既定の `copy-on-select = clipboard`・`middle-click-action = clipboard-paste`。→ [config](../platform/config.md)）——選択の 1 アクションでクリップボードに入る手順の短さを Orbe の価値として保つ。マウス位置は tracking area で伝える（enter で viewport 内に位置確立・exit で範囲外座標）。
