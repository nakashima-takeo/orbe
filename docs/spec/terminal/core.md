...
- **クリップボード**: 端末アプリ発の読み取り（OSC 52 read・Kitty clipboard read）は Orbe 既定の `clipboard-read = deny`（→ [config](../platform/config.md)）で libghostty が拒否し、host には届かない——クリップボード内容の情報漏洩を防ぐため。host に届く読み取りはユーザー発のペーストとその型一覧で、扱う表現は `text/plain` だけ。Orbe は確認 UI を持たないので、確認が要る操作のうち通すのはペーストだけ（確認済みとして完了）で、それ以外は拒否する。ユーザーが `clipboard-read = ask` にすると端末アプリ発の読み取りがこの確認に回り、中身は渡らないが応答は `deny` と異なる——OSC 52 は中身があれば空の応答・空なら無応答、Kitty read は中身があれば EPERM・空なら成功で、クリップボードが空かどうかが端末アプリに分かる（型一覧だけの Kitty read は確認を経ず応答される）。`ask` でも ⌘V に続く Kitty paste event（mode 5522）のパスワード付き read は許可済みとして確認なしで通る。既定を `deny` にしているのは、この差も見せないため。書き込みも同じ論理で、確認が要る書き込み（user が `clipboard-write = ask` にした場合）は通さず、`text/plain` 表現だけを NSPasteboard へ置く。
  - **制約: Kitty clipboard protocol の paste event（mode 5522）を有効にした端末アプリには ⌘V でペーストできない。** mode 5522 では ⌘V で host が型一覧と使い捨てパスワードを送り、端末アプリがそのパスワード付きの OSC 5522 read で中身を取りに来る。libghostty は `clipboard-read = deny` のとき、パスワードによる許可を見る前にこの read を拒否する。端末アプリには「クリップボードが空か」「どの型があるか」も見せない方針を優先した結果として受け入れている。ユーザーが `~/.config/ghostty` で `clipboard-read` を変えればこの限りではない。
