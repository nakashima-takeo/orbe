...
- **クリップボード**: 端末アプリ発の読み取り（OSC 52 read・Kitty clipboard read）は Orbe 既定の `clipboard-read = deny`（→ [config](../platform/config.md)）で libghostty が拒否し、host には届かない——クリップボード内容の情報漏洩を防ぐため。host に届く読み取りはユーザー発のペーストとその型一覧で、扱う表現は `text/plain` だけ。`PRIMARY`（X11 の選択クリップボード）の読み取りは非対応と答える——macOS に無いものを general の中身で代用しない（`SELECTION` は ⌘⇧V の `paste_from_selection` が使うので general に向ける）。Orbe は確認 UI を持たないので、確認が要る操作のうち通すのはペーストだけ（確認済みとして完了）で、それ以外は拒否する。ユーザーが `clipboard-read = ask` にすると端末アプリ発の読み取りがこの確認に回り、中身は渡らないが応答は `deny` と異なる——OSC 52 は中身があれば空の応答・空なら無応答、データだけを求める Kitty read は中身があれば EPERM・空なら成功（型一覧も併せて求める read は空でも EPERM）で、クリップボードが空かどうかが端末アプリに分かる（型一覧だけの Kitty read は確認を経ず応答される）。`ask` でも ⌘V に続く Kitty paste event（mode 5522）のパスワード付き read は許可済みとして確認なしで通る。既定を `deny` にしているのは、この差も見せないため。書き込みも同じ論理で、確認が要る書き込み（user が `clipboard-write = ask` にした場合）は通さない。書き込みでテキストと見なすのは macOS が plain text と解する MIME（`text/plain`・`text/plain;charset=utf-8` 等。Kitty write の MIME は正規化されず端末アプリの書いた名前のまま届くため、判定は macOS の型システムに委ねる）で、最初のテキスト表現を NSPasteboard へ置き、テキスト表現が無い書き込みは無視する（クリップボードを消さない）。表記の揺れをどこまで吸収するかも macOS の型解決に委ね、Orbe は MIME 文字列を自前で分解しない——`text/plain; charset=UTF-8` のように `;` の後に空白を含む MIME は `UTType(mimeType:)` が動的型に解決してテキストと判定されず、その書き込みは無視される（クリップボードは変わらない）。例外は UTF-16 系の plain text（`text/plain;charset=utf-16` 等）で、macOS はテキストと解するが Orbe は本文を UTF-8 としてしか読まないため、テキスト表現とは見なさない——その MIME だけの書き込みはクリップボードを変えない（NUL 混じりの文字列を置かない）。
