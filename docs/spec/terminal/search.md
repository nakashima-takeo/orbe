...
⌘F でフォーカス中のペインに検索バーをオーバーレイする。needle を入力すると逐次検索し、Enter で次 / Shift+Enter で前へ循環ジャンプ、Esc で終了してターミナルへフォーカスを返す。件数は selected/total で表示する。表示中にもう一度 ⌘F を押した場合は入力欄への再フォーカスのみで、バーを二重生成しない。
