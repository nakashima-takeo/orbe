...
- `report_agent {paneId, agent, state, sessionId?, message?}` … エージェント hook の状態報告を発信元ペインへ適用する（[agent-notify](agent-notify.md)）。`state=="clear"` で状態/コマンド/セッション ID/文言/状態変化時刻を nil、それ以外は state/command/message を立て sessionId があれば更新（状態変化時刻は state が実際に変わったときだけ進む）。
