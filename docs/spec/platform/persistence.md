...
- タブ 1 枚分の復元単位は、閉じたセッションの復元（⇧⌘T パレット → [closed-agents](../palette/closed-agents.md)、制御 API `restore_sessions`）と共有する——同じ経路を通るので、戻るもの／戻らないものが一致する。閉じたセッションの記録はこのファイルではなく[寿命ログ](session-log.md)が持つ。
