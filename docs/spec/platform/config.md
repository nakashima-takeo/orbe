...
- **`copy-on-select = clipboard`・`middle-click-action = clipboard-paste`** … 選択するだけで NSPasteboard に入り、中クリックで NSPasteboard の文字列をペーストする（→ [terminal/core](../terminal/core.md)）。libghostty の macOS 既定（`none`・`primary-paste`）は selection クリップボードを前提にしており、それを持たない Orbe では選択コピーも中クリックも無反応になるため、Orbe の既定として明示する。
