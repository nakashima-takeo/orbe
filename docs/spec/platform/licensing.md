...
依存を追加・変更したら、`NOTICE` のエントリと `licenses/` の全文を追随させる（`vendor/ghostty` の SHA 更新時・`Package.resolved` 更新時が対象）。`licenses/` はコミット原本であり、手で追随する。

**網羅の確認は配布バイナリから行う。** `nm -a` で `Contents/MacOS/Orbe` の OSO デバッグマップを全列挙し、アーカイブメンバごとに出所を帰属させる。`build.zig.zon` の依存一覧は網羅の根拠にならない——`.lazy = true` の依存は構成によってリンクされず（HarfBuzz は macOS/CoreText 構成では入らない）、逆に lazy でも無条件にリンクされるもの（libintl・dcimgui）があるため。
