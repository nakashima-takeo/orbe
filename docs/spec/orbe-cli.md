...
control.sock の解決順は `ORBE_SOCK`（ペイン注入の絶対パス・最優先）→ `ORBE_STATE_DIR/control.sock` → 既定の Application Support 直下（自ビルドのチャネルが焼いた bundle id・[channel](channel.md)）。pane/tab は現ペイン既定に `ORBE_PANE`（ペイン注入の自 pane id）を読む。config/ws はインスタンス/WS 単位なので `ORBE_PANE` を読まない。外部（`ORBE_PANE` 無し）で pane/tab の対象を省略すると usage エラー（exit 2）。
