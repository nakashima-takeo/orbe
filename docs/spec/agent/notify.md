...
「エージェントの状態をターミナルの基本情報にする」を支える配管。エージェント CLI の hook が現在の状態とセッション ID を**制御ソケットへ報告**し（[control/api](../control/api.md) の `report_agent`）、発信元タブの in-memory 状態に保持する。送信は `.app` 同梱 CLI `orbe-report` が担う。配布物（claude/codex/agy 兼用プラグイン）の構造と導入は [plugin-package](plugin-package.md)。

経路: hook → 配布物内の薄いシム → 同梱 `orbe-report` へ `exec` 委譲 → 制御ソケットへ JSON-RPC 1 行 → `report_agent` を受けて `tabId` で発信元タブを解決 → 状態 / コマンド名 / セッション ID を保持。

## tab identity（env で運ぶ）

tab identity は env で運ぶ（tty 経路を要さない）。Orbe は全タブに、materialize 開始時（surface 生成の前）に注入する。
...
- `ORBE_TAB`・`ORBE_SOCK` のどちらかが無ければ即 no-op（Orbe 外）。Orbe が動いておらず socket 接続に失敗しても no-op。
...
- サブエージェント実行中の hook payload（親と同じセッション ID で届き、サブエージェント識別子を持つ）は報告しない——タブのセッション状態ではないため。socket へ接続する前に落とす。
  - 既知の制約: permission 待ちの通知はサブエージェント識別子を**持たない**ので、サブエージェント内の承認要求でもタブは `waiting` になる。一方その待ちを解く側（バッチ解決の報告）はサブエージェント識別子を持つので落ちる。結果、サブエージェント内で承認しても `waiting` は即座には解けず、親のバッチが解決した時点まで残る。
...
状態変化時刻は **state の値が実際に変わったときだけ** now に進める（同値の連続報告で Attention 一覧の並びが暴れない）。タブ消滅で保持値も消える（本配管はファイルに置かない。永続は snapshot 側 → [persistence](../platform/persistence.md)）。hook 外の書き込みは、復元時の再設定（[persistence](../platform/persistence.md)）と、`idle` への書き戻し——done のフォーカス消費とタブのコンテキストメニューによるリセット（[chrome](../chrome/chrome.md)）——のみ。`idle` への書き戻しは state だけを変え、同一性・文言・状態変化時刻は進めない。状態変化は `agent_state` イベントを emit し、chrome 更新と制御 API の履歴（[control/api](../control/api.md)）へ流れる。イベントが運ぶのは状態語（報告が消えたときは `clear`）と、その遷移の時点でペインが持つ文言・session id。`waiting` / `done` への実変化は通知音へも流れる（→ [sound](sound.md)）。
