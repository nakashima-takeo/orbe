...
状態・セッション ID・コマンド名・文言（出所つき）・状態変化時刻はペインの in-memory 状態。`clear` で全部 nil。`clear` 以外の更新は state/command を更新し、session_id は新値があれば更新・無ければ維持する——状態遷移をまたいで resume 可能な session_id を保つため。
