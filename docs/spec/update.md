...
サイレント確認 → 自動ダウンロード＋検証（EdDSA とコード署名。Sparkle が実施）→ **再起動待ちになった瞬間に一度だけ**右下トースト → 「今すぐ再起動」＝即時適用＆再起動 / 放置・✕・「閉じる」＝終了時に自動適用。更新が無ければ最新として最終確認時刻とともに残る。確認中・ダウンロード中・失敗は通知せず、設定の状態カードにだけ現れる。トーストは 1 バージョンにつき一度だけで、**明示的に閉じるまで残る**（✕・「今すぐ再起動」・「変更内容」でシートを開くと閉じる。時間では消えない——適用の機会は再起動/終了時で、起動しっぱなしの端末では時限通知が役目を果たさないため）。
...
- **設定›アップデート**（[settings-palette](settings-palette.md)）: 状態カード（まだ確認していない（現在バージョンと最終確認時刻を添える。一度も確認していなければ「—」）/ このビルドでは確認しない（updater が起動ゲートを通らなかったとき。バージョンも時刻も出さない）/ 確認中 / ダウンロード中（進捗と受信量）/ 最新です / 失敗（再試行）/ 適用待ちの 7 状態）＋現在バージョンと最終確認時刻＋トグル 3 種＋「今すぐ確認」。Single Source of Truth——トーストを見逃しても状態はここに残る。

「今すぐ確認」（失敗カードの「再試行」も同じ導線・同じ可否）は確認を受け付けられない間は実行しない。可否は 3 値で、実行できない理由まで持つ——**セッション進行中**（確認中・ダウンロード中・適用待ち・背景の定期確認中）と、**updater が動いていない**（起動ゲートを通らなかったビルド）を区別する。行の表示は、背景の定期確認が走っている間だけ「アップデートを確認中…」を名乗り（カードはまだ結果を持たないので、確認が走っていることは行だけが語る）、残り（ダウンロード中・適用待ちは隣のカードが理由を持つ／updater 不活性は確認が走らない）は減光する。

App メニューに「更新を確認…」があり、設定の「今すぐ確認」と同一導線（設定パレットのアップデートセクションを開いて確認を走らせ、結果は状態カードに現れる）。

「今すぐ再起動」（トースト・状態カード・変更内容シートが共有）の押下は必ず着地する。着地先は優先順に、終了要求の再送 / サイレント staged の即時適用 / 保留中の応答へのインストール指示 / dismiss 済みセッションの resume のいずれかで、どれも取れない（セッション進行中）ときは押下を預かり、着地できる瞬間（セッションが空く・サイレント staged の即時適用ハンドラが届く）に自分で適用へ進む。適用要求を立てて他のセッションに拾わせることはしない——要求がセッション境界を越えて残らず、頼んでいないタイミングで終了・再起動しない。終了確認ダイアログをキャンセルしても、押し直せば終了要求が再送される（何度でも）。
