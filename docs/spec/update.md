...
- **変更内容シート（中央モーダル）**: トーストと設定の「変更内容」が同じここへ着地する。見出し「vX.Y.Z の変更内容」・日付とサイズ・リリースノート（appcast description の Markdown を、見出し＝分類、箇条書き＝項目、それ以外の段落＝マーカー無しの本文として描画。この 3 種以外のブロック——番号付きリスト・コードブロック・引用・表——は描かない。分類の色とマーカーは**見出しの語**で決まり（`新機能`＝青 `＋` / `改善`＝黄 `＋` / `修正`＝muted `✓` / 規約外・見出し無し＝中立 `•`）、並び順・個数には依らない。カード高は窓高（上下 20pt の余白を残す）で頭打ちし、超えた分はノート部だけが内部スクロールへ回る——見出し・検証済み行・ボタンは常に見える。短いノートでは内容ぴったりの高さになる）・「Developer ID 署名と公証を検証済み」行・「再起動して更新」「閉じる」。scrim タップ / Esc / 閉じるは同じ着地（終了時適用のまま）。↵ は「再起動して更新」。設定パレットから開いた場合は閉じるとパレットへ戻る。
