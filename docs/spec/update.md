...
「今すぐ確認」（失敗カードの「再試行」も同じ導線・同じ可否）は確認を受け付けられない間は実行しない。可否は 3 値で、実行できない理由まで持つ——**セッション進行中**（確認中・ダウンロード中・適用待ち・背景の定期確認中）と、**updater が動いていない**（起動ゲートを通らなかったビルド）を区別する。行の表示は、状態カードが理由を語らない背景の定期確認中だけ「アップデートを確認中…」を名乗り、残り（ダウンロード中・適用待ちは隣のカードが理由を持つ／updater 不活性は確認が走っていない）は減光する。
