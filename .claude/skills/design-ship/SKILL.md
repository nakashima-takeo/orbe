...
`.claude.local/design-ledger/<kebab>.md` を作る（以下デザイン台帳）。デザインと実装が食い違った点は、判明し次第すべて「見本: X / 実装: Y / 理由: Z / 出典: <モックのファイル>」の形でここに追記する。

### 3. ship に乗る

`Skill` ツールで `ship` を読み込み、デザイン抽出を題材（完了条件の材料）にしてそのフローで実装〜源流へのマージまで運ぶ。単一 PR か複数 PR かは ship の受付で決まる。重ねる差分は2つ:

- **完了条件に「デザイン抽出との一致」を含める。** 理由なき逸脱は不一致。理由のある逸脱だけがデザイン台帳に残る。複数 PR に分けるときは、全体計画で各単位の完了条件にデザイン抽出の該当部分を割り当て、複数単位が揃って初めて成立する見た目は統合後の確認条件に置く。
- **成果物に触れる作業者への委譲プロンプトに逸脱報告を義務付ける。** 計画・実装・レビュー・視覚確認、複数 PR では統合レビューも。委譲するたび「デザイン抽出から逸脱した点は『見本: X / 実装: Y / 理由: Z』の形で要約に含めよ」を足し、返ってきた逸脱をデザイン台帳へ移す。visual-check の「残した逸脱と理由」もデザイン台帳に入れる。

## 逆反映

実装モードでは源流へのマージ後（複数 PR では最終統合の後）に、逆反映のみモードでは受付の直後にここへ来る。

1. **変更一覧を確定する。** 実装モードではデザイン台帳が一覧の元。ただし台帳の項目は後の工程や後の単位で見本どおりに戻されていることがあるので、各項目をマージ後の実装（Swift ソースと `.preview/gallery/*.png`）と突き合わせ、解消済みの項目は落とす。残りが空なら「逆反映なし」と報告して終了。逆反映のみモードでは、対象範囲のモックと本番（Swift ソースと `.preview/gallery/*.png`。本番が正）を突き合わせ、ズレを「見本: X / 実装: Y / 出典: <モックのファイル>」で列挙する。
