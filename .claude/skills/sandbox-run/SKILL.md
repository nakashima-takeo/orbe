...
3. モードで分岐:
   ...
   - **無人モード**: まず**煙探知を 1 本通してから** `$ORBE_STATE_DIR/control.sock` を呼び出し側へ渡し、制御 API で駆動して確かめる。煙探知は「`list_panes` でペインを取り、`send_text "echo L4D\"\"ONE_<id>"`（`<id>` は毎回ランダム）＋ `send_key enter` を送り、`get_pane_text` に連結された `L4DONE_<id>` が現れるまでポーリング」。目印をコマンド行の中で 2 つのリテラルに割るのが要点で、連結形はシェルが引用符除去を**評価した**出力にしか現れない——入力行の描画回数を数える形はプロンプトのテーマ次第で入力側だけでも増えるが、ここは実 `.app` ＝利用者の rc とテーマが走る環境なので、その依存を持ち込まない。これが通らないなら、起こしたバンドルは制御 API から駆動できていないので、駆動を始めず失敗として呼び出し側へ返す。`.app` の起動経路と `AppDelegate` の配線は `swift test` の守備範囲外なので、機械的に確かめる場所はここしかない。
